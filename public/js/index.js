...
const testApi = async () => {
  ...
  // Additional test for user class
  //let user = await User.loadOrCreate("mchang");
  //console.log(user);
  //await user.getFeed();
  //await user.makePost("Something cool!");
  //await user.save();
  //await User.loadOrCreate("shrestp");
  //await user.addFollow("shrestp");
  //await user.save();
  //console.log(user);
  //await user.deleteFollow("shrestp");
  //await user.save();
  //console.log(user);
};
